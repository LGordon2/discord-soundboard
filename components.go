...
const addSoundCardComponentTmplRaw = `
    <div
        ...
        <div class="flex flex-row">
            ...
            <button class="flex shrink items-center justify-center enabled:text-green-500 disabled:text-gray-500" hx-on="htmx:beforeProcessNode: window._iconLoad(this, 'plus')" {{if .disabled}}disabled="true"{{else}} hx-post="/add-sound?soundLocation={{ .soundName }}&guildID={{ .guildID }}"{{end}}></button>
        </div>
    </div>
`
