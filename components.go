...
func toggleComponent() string {
	var buf bytes.Buffer
	err := template.Must(template.New("toggleComponentTmpl").Parse(toggleComponentTmpl)).Execute(&buf, nil)
	if err != nil {
		panic(err)
	}
	return buf.String()
